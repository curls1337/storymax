...
for (const sc of SCENARIOS) {
  ...
  for (let pageNum = 1; pageNum <= sc.pageCount; pageNum++) {
    ...
    const hasContinuity = /VISUAL CONTINUITY:/i.test(prompt);
...
console.log('\n' + '#'.repeat(78));
console.log('SUMMARY  (len<=2000; CONSTRAINTS/NO_NEG/FOOT/SCENES/CAM; brandOk; fidelityOk; seqOk)');
console.log('#'.repeat(78));
let allOk = true;
for (const r of rows) {
  ...
  if (!ok) allOk = false;
  console.log(
    `${ok ? 'OK ' : 'BAD'}  ${r.id.padEnd(22)} ref=${String(r.ref).padEnd(5)} p${r.page.padEnd(4)} len=${String(r.len).padStart(4)}  SCENES=${r.hasScenes} brandKept=${r.brandKept} constr=${r.hasConstraints} noNeg=${r.noLiteralNegative} window=${r.hasWindow} cont=${r.hasContinuity}`
  );
}
