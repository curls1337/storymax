...
const STYLES = {
  ...
  cube_box_transform: {
    name: 'Cube Box Transformation', desc: 'Sebuah tangan menekan tombol di kubus mekanis premium lalu melemparnya → kubus otomatis mekar & morph mulus (mekanis, tersambung, tanpa bagian lepas) jadi produk/model → hero shot. Kamera lebar & stabil, fotorealistis sinematik ala video viral.',
    category: 'Transformasi & Reveal', format: '9:16', faceMode: 'faceless', bg: 'light',
    camera: 'cinematic photorealistic reveal on ONE stable, WIDE, locked-off camera (no sudden cuts or jumps); a hand only presses a button then tosses the cube, after which it auto-unfolds hands-free; shallow DOF (NOT CGI, NOT a glowing robot)',
    ...
    arc: ['a small premium mechanical cube rests on a surface; a single hand presses a button on top of it (the only hand action)', 'the hand gently tosses / flips the cube, then leaves the frame', 'hands-free, the cube automatically unfolds — armored panels slide & telescope outward smoothly, mechanically connected, no loose parts', 'the mechanism seamlessly reshapes into the subject itself (the product, or a scaled model/structure of it)', 'the finished subject in a calm, wide premium hero shot'],
    negatives: NEG.concat(['humanoid robot / mecha / Transformer (it becomes the product, not a robot)', 'exploding, flying or detached parts', 'energy beams, glow or magic FX', 'CGI cartoon or anime look', 'redesigned or renamed product', 'a full person or human face in frame', 'sudden camera cuts, jumps or shaky repositioning']),
  },
  shape_morph_transform: {
    name: 'Adaptive Shape Transformation', desc: 'Wadah mekanis presisi (bentuk otomatis menyesuaikan subjek: bola, kubus, silinder, prisma segitiga, atau balok) diletakkan di permukaan → mekar & morph mulus otomatis jadi produk/model (mekanis, tersambung, tanpa bagian lepas), tampilan TETAP mekanik/logam presisi dari awal sampai akhir (bukan mainan anak/kartun/plastik) → hero shot. Fotorealistis sinematik.',
    ...
    negatives: NEG.concat(['exploding, flying or detached parts', 'energy beams, glow or magic FX', 'CGI cartoon or anime look', 'turning into a plastic, childish or kids-toy look', 'changing material or finish midway (must stay high-tech metal / mechanical throughout)', 'redesigned or renamed product', 'hands, human hands, fingers, holding hand, person, human']),
  },
  asmr_toy_transform: {
    name: 'ASMR Toy Transform (Static)', desc: 'Kubus diletakkan di meja lalu membuka sendiri jadi mainan die-cast mini PREMIUM/mahal (logam berkualitas, cat halus, detail realistis) — kamera DIAM total, suara ASMR mekanis, gerak smooth.',
    category: 'Transformasi & Reveal', format: '9:16', faceMode: 'no_people', bg: 'light',
    ...
    arc: ['a small armored cube (product emblem/button on top) rests statically on a worn white table', 'the cube rests still, then its panels begin to UNFOLD by themselves — smooth, satisfying mechanical motion', 'panels slide, hinge and telescope out step by step, building the shape (ASMR clicks & servo whirs)', 'it forms a HIGH-END, PREMIUM, expensive-looking miniature die-cast collectible of the product on the same table', 'the finished premium die-cast collectible (heavy metal feel, flawless paint, glossy finish) rests on the table, held static'],
    negatives: NEG.concat(['any camera movement, pan, zoom, orbit or shake', 'exploding or flying parts', 'glow or energy', 'life-size (must be a small tabletop toy)', 'cheap hollow plastic or low-quality toy look', 'hands, human hands, fingers, holding hand, person, human']),
  },
  unboxing: {
  ...
  kids_education: {
    name: 'Edukasi Anak (Playful)', desc: 'Gaya edukasi anak ceria: warna cerah, karakter kartun lucu, ikon belajar (buku, bintang, topi wisuda) & font playful; cocok konten/produk anak.',
    category: 'Proses & Edukasi', format: '9:16', faceMode: 'full', bg: 'light',
    camera: 'bright playful flat-illustrated composition with cute cartoon characters/mascots, big rounded shapes and friendly learning icons (illustrated, NOT a photo)',
    lighting: 'bright cheerful even lighting, vibrant candy/primary palette', header: 'STORYBOARD — EDUKASI ANAK',
    layoutHint: 'a grid of {N} playful rounded numbered cards, each with a cute cartoon mascot, learning icons (books, stars, graduation cap) and a big friendly callout',
    arc: ['cheerful hook introducing the topic with a smiling mascot', 'introduce the concept using cute icons & simple visuals', 'a fun, relatable example for kids', 'the benefit / what they learn, celebrated', 'happy recap with encouragement / call to action'],
    negatives: NEG.concat(['scary, dark or somber imagery', 'photorealistic look (must be a bright friendly cartoon)', 'complex or cluttered text', 'adult or serious tone']),
  },
  tiktok_text_ad: {
    name: 'TikTok Text Ad (Caption)', desc: 'Iklan vertikal sinematik dengan CAPTION teks on-screen besar tiap scene (hook → fitur → bukti → CTA). Foto produk bersih, gaya ala TikTok/Reels.',
    category: 'UGC & Social', format: '9:16', faceMode: 'chin_max', bg: 'light',
    camera: 'clean cinematic product photography, ~35mm, shallow depth of field, ONE clear subject per panel',
    lighting: 'warm cinematic softbox light, appetizing & premium, crisp product highlights', header: 'STORYBOARD — TIKTOK TEXT AD',
    layoutHint: 'a grid of {N} numbered panels; EACH panel pairs the scene photo with a BOLD on-screen caption (big kinetic social-video lettering, high contrast + clean outline) and a tiny time chip',
    arc: ['hook: a relatable problem or scroll-stopping attention grab', 'reveal the product and its key feature', 'show it in action / the main benefit', 'social proof or a standout detail', 'clear call to action (order now / where to buy)'],
    negatives: NEG.concat(['garbled or misspelled caption text', 'mismatched ransom-note fonts', 'cluttered overlapping text', 'fake watermarks or platform UI']),
  },
  diy_build: {
