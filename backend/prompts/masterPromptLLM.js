...
6c. VOICE OVER NOTE — if PARAMS.voiceOver is TRUE, print a small "VO:" note under EACH panel with a SHORT one-line voice-over cue for that scene in PARAMS.voLanguage (a few words, caption-style, correctly spelled, NOT a paragraph) so the storyboard itself carries the narration cue. If FALSE, do NOT add any VO note.
...
async function generateMasterPromptWithAI(spec, ctx, db) {
  try {
    const {
      ...
      voiceOver = false, voLanguage = 'Bahasa Indonesia',
    } = ctx;
