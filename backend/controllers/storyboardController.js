...
function getEnhancedPrompt(style, userPrompt, gridCount = 6, showFace = false, startScene = 1, totalDuration = 60, secondsPerPage = 15, hasRefImage = false, containerShape = 'auto') {
  ...
  if (style === 'cube_morph_product') {
    resultPrompt = `A professional 3D Cube Morph & Product Transition storyboard presentation sheet, vertical layout, clean dark-charcoal background. At the top header, a prominent gold-and-white header banner reading 'STORYBOARD - 3D CUBE MORPH TRANSITION' and the bold target product name in center with 'PART ${pageNum}', a 3D isometric cube wireframe diagram on the right, and metadata badges showing 'STYLE : 3D TRANSITION', 'ROTATION : MULTI-AXIS', 'ASPECT RATIO : 9:16'. The main content is a structured 2-column comparison layout displaying scenes ${startScene} to ${endScene}: ${finalPromptText}. 

CRITICAL VISUAL RULES:
1. For each scene row, draw exactly two side-by-side widescreen 16:9 image panels: on the left, a close-up of a futuristic dark metallic mechanical cube (in different states of opening/morphing matching the scene); on the right, a close-up of the target product (matching the scene details) on the exact same tabletop. 
2. Show a gold arrow pointing from the left panel to the right panel.
3. Do NOT write or display any text blocks, prompt descriptions, code text, or labeled text columns next to the image panels. The sheet must only contain the header, the grid of clean visual panels with scene numbers (${startScene}, ${startScene+1}...) on the left, and a clean technical footer. Absolutely no prompt labels or text paragraphs on the poster canvas.
