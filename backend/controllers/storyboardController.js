...
function getEnhancedPrompt(style, userPrompt, gridCount = 6, showFace = false, startScene = 1, totalDuration = 60, secondsPerPage = 15, hasRefImage = false, containerShape = 'auto') {
  ...
  if (style === 'cube_morph_product') {
    return `A professional 3D Cube Morph & Product Transition storyboard presentation sheet, vertical layout, clean dark-charcoal background. At the top header, a prominent gold-and-white header banner reading 'STORYBOARD - 3D CUBE MORPH TRANSITION' and the bold target product name in center with 'PART ${pageNum}', a 3D isometric cube wireframe diagram on the right, and metadata badges showing 'STYLE : 3D TRANSITION', 'ROTATION : MULTI-AXIS', 'ASPECT RATIO : 9:16'. The main content is a structured comparison layout displaying scenes ${startScene} to ${endScene}: ${finalPromptText}. For each scene, it shows three distinct prompt blocks:
1. 'FIRST IMAGE PROMPT' detailing: a highly detailed close-up shot of a futuristic mechanical cube with a dark matte metallic finish, angular panel lines, circular button core, being held by hands or sitting on a worn tabletop.
2. 'SECOND IMAGE PROMPT' detailing: a highly detailed close-up of the target product (matching the scene description) placed on the exact same worn tabletop, with soft lighting and natural reflections.
3. 'IMAGE TO VIDEO PROMPT' detailing: extreme close-up of the mechanical cube, button press activates internal mechanisms, panels split, rotate, and extend with visible pistons, gears, and hinges, reassembling into the final target product within 5 seconds with a smooth seamless transformation and high precision motion.
At the bottom, a technical rendering spec footer detailing 'TRANSITION CURVES', 'MOTION BLUR SPECS', '3D ENGINE RENDER', and 'AUDIO CLICK TIMING'. High-contrast design, clean borders, professional tech look. ${faceClause}. --ar 3:4`;
  ...
  if (style === 'tiny_workers_miniature') {
    return `A professional Pixar-style 3D animated storyboard presentation sheet, vertical layout. At the top header, a prominent brand logo on the left, a bold title 'STORYBOARD - Tiny Workers' with the scene description in center with 'PART ${pageNum}', a widescreen Pixar 3D illustration of a food truck/workshop scene on the right, and metadata badges showing 'DURASI 15-20s', 'ASPECT RATIO 9:16 Vertical', 'GAYA Pixar-style 3D Animation'. The main content is a structured 2x3 (6-panel) grid layout displaying scenes ${startScene} to ${endScene}: ${finalPromptText}. Each panel displays a widescreen 3D animated scene showing tiny toy workers (Lego/Pixar style) in yellow builder helmets crafting miniature objects, a dark-green square scene number badge (1, 2, 3...) at the top-left, a duration badge (e.g. '0-3s') at the top-right, and a clean text description card below with a tiny outline icon on the left (e.g. leaf, bowl, fire, drop, star icon). At the bottom, a prominent aesthetic spec footer detailing 'WARNA UTAMA' (color swatches), 'SUASANA', 'PERSEKITARAN', 'PENCAHAYAAN : Kuning hangat', dan 'FOKUS : Proses penyediaan'. Cute 3D animation style, warm cozy lighting, clean cartoon aesthetic. ${faceClause}. --ar 3:4`;
  }

  if (style === 'diy_build_process') {
    return `A professional DIY build process and miniature crafting storyboard sheet, vertical layout stack. At the top header, a clean black-and-gold outline banner reading 'STORYBOARD – DIY BUILD PROCESS' with product fields showing 'DIY ITEM', 'STYLE : MINIATURE HOUSE', 'COMPLEXITY : SIMPLE'. The body features stacked scene rows for scenes ${startScene} to ${endScene}: ${finalPromptText}. Each row displays: on the left, a large gold scene number (01, 02...) and timestamp (e.g. '0:00-0:02'); in the center, a widescreen 16:9 illustration of the miniature building step; on the right, a detailed technical metadata block with 6 rows: 'VISUAL SHOT', 'CAMERA MOVEMENT', 'ACTION', 'DETAIL FOCUS', 'SFX / ASMR', and 'TRANSITION'. At the bottom, a prominent footer detailing 'GLOBAL VISUAL PROMPT', 'GLOBAL ASMR PROMPT', a 'CAMERA DISTRIBUTION' pie chart, a 'SHOT LIST SUMMARY' table, and a 'SHOT HIGHLIGHTS' sub-footer bar. Elegant technical DIY design, gold/orange accents, clean dark borders. ${faceClause}. --ar 3:4`;
  }

  if (style === 'clean_step_card_grid') {
    return `A professional e-commerce product step-by-step usage storyboard poster, vertical layout, clean bright white background with a modern purple accent theme. At the top header, a clean white banner reading 'STORYBOARD – [PRODUCT TITLE] | Cara Guna Mudah & Praktikal' on the left, a purple rounded pill badge on the right reading 'DURASI : 10s', with thin gray separator lines. The main content is a structured 5x2 (10-panel) grid layout of clean rounded white card panels displaying scenes ${startScene} to ${endScene}: ${finalPromptText}. Each card panel displays a widescreen 16:9 product usage photo, a purple square scene number badge (1, 2, 3...) at the top-left, a purple pill timestamp badge (e.g. '0:00-1s') at the top-right of the photo, and a text block below with a bold purple title (e.g. 'UNBOXING', 'LANGKAH 1') and a 1-2 sentence description of the action. At the bottom, a prominent dark purple footer banner reading '📢 MESEJ UTAMA: Mudah digunakan, comel & praktikal – Nasi sedap setiap hari!' on the left, and a brand official store logo on the right. High-contrast clean design, sharp sans-serif typography, bright studio lighting. ${faceClause}. --ar 3:4`;
  }

  if (style === 'recipe_cooking_table') {
    return `A professional recipe and cooking tutorial script table presentation sheet, vertical layout. At the top header, a prominent dark banner reading 'RECIPE STORYBOARD & ASMR TUTORIAL' and the bold food dish/recipe title in center with 'PART ${pageNum}', and metadata fields showing 'TOTAL TIME : 10 DETIK', 'SCENE COUNT : ${endScene}', 'ASPECT RATIO : 16:9'. Below is a high-contrast 8-column storyboard table containing columns titled: 'TIME', 'SCENE', 'VISUAL DESCRIPTION', 'CAMERA', 'CONTINUITY', 'ASMR AUDIO', 'TRANSITION', and 'NOTES'. The table rows display scenes ${startScene} to ${endScene} showing: ${finalPromptText}. Column 1 shows time intervals (e.g. '00:00 - 00:01'). Column 2 shows scene steps (e.g. 'Raw Chicken Preparation', 'Deep Fry Chicken'). Column 3 displays widescreen 16:9 cooking photos with a short visual action description. Column 4 shows camera shot angles (e.g. 'Close-up (Slow Push In)', 'Close-up (Macro)', 'Close-up (Top Angle)'). Column 5 describes continuity status. Column 6 specifies detailed ASMR sounds (e.g. 'oil sizzling and bubbling sound', 'flour crunch and coating sound'). Column 7 shows transition types (e.g. 'Match cut', 'Cut on action'). Column 8 displays technical notes. Professional food studio lighting, clean table borders, high contrast. ${faceClause}. --ar 3:4`;
  }

  if (style === 'character_design_turnaround') {
    return `A professional character and mecha design concept turnaround sheet, vertical layout sheet. At the top header, a bold red-and-white header banner reading 'STORYBOARD - CHARACTER DESIGN' and 'HENSHIN SEQUENCE SHEET' with the product/character title, an illustration of 'DRIVER BELT' or transformation device, and metadata boxes detailing 'THEME CONCEPT', 'HENSHIN CODE', 'GENRE', and 'ASPECT RATIO : 16:9'. The main body is a structured 4x4 (16-panel) sequence grid displaying scenes ${startScene} to ${endScene}: ${finalPromptText}. Each sequence panel contains a grunged hand-drawn anime sketch illustration showing parts attaching or turnaround views (Front/Back/Side View), a red square scene number badge (1, 2, 3...) at top-left, a bold scene title, description text, and diagram pointer callouts (e.g. 'FRONT WING', 'NOSE CONE', 'GLOWING VISOR'). At the bottom, a prominent technical breakdown spec footer detailing 'PARTS BREAKDOWN TO ARMOR' with individual parts slots, a 'SPEC DATA' table (Height, Weight, Punch/Kick Power, Max Speed), and a 'FINISHER' layout box. High-contrast sketch design, sharp typography, technical concept art style. ${faceClause}. --ar 3:4`;
  }

  if (style === 'comic_grunge_storyboard') {
    return `A professional short story comic storyboard presentation sheet, vertical layout, heavily stylized with a textured grunged/dirty paper aesthetic. At the top header, a prominent red-brown banner showing 'STORYBOARD FILM PENDEK' and 'KOMEDI + SATIR' on the left, a giant bold comic title in center with 'PART ${pageNum}', a ripped cardboard note box showing 'KONSEP CERITA', and metadata labels showing 'GENRE : KOMEDI + SATIR', 'DURASI : 10 DETIK', 'ASPEK RASIO : 16:9', 'GAYA VISUAL : GRUNGED ANIME STYLE'. The main content is a structured 3x3 (9-panel) comic grid displaying scenes ${startScene} to ${endScene}: ${finalPromptText}. Each comic panel features a grunged hand-drawn anime/manga sketch illustration, a black square scene number badge (1, 2, 3...) at the top-left, a timestamp badge (e.g. '0:00 - 0:01') at the top-right, a sketchy Speech Bubble with cartoon text, a bottom-aligned 'TEKS OVERLAY: [Handwritten Bold Text]' block, and a bottom 'VOICE OVER: [Narration Quote]' black strip. At the bottom, a heavy grunged footer banner displaying 'CATATAN VISUAL', 'PALET WARNA' (with color swatch blocks), 'ELEMEN VISUAL' (with icons for coretan, stiker robek, noda cat, stempel), 'GAYA TEKS OVERLAY' (Handwritten, grunge highlight), and a giant yellow badge showing 'DURASI TOTAL: 10 DETIK'. Grungy manga sketch style, hand-drawn aesthetic, high-contrast shadows. ${faceClause}. --ar 3:4`;
  }

  if (style === 'ugc_product_showcase_grid') {
    return `A professional UGC product showcase commercial storyboard sheet, vertical layout, clean bright white and pastel-colored background. At the top header, a clean pink/red accent banner reading 'STORYBOARD - UGC PRODUCT VIDEO' and the bold product title in center with 'PART ${pageNum}', and metadata fields showing 'DURASI TOTAL : 10 DETIK', 'JUMLAH SCENE : ${endScene}', 'ASPECT RATIO : 16:9'. The main content is a structured 3x3 (9-shot) grid layout displaying scenes ${startScene} to ${endScene}: ${finalPromptText}. Each grid panel displays a widescreen 16:9 bright product B-roll photo, a pink/light-red square scene number badge (1, 2, 3...) at the top-left, a pink timestamp badge (e.g. '0.0 - 1.0 DETIK') at the top-right, and a structured metadata block below with 3 fields: 'VISUAL : [Description]', 'AKSI : [Action Description]', and 'KAMERA : [Camera Movement]'. At the bottom, a prominent 5-box technical summary footer detailing 'INFORMASI TEKNIS', 'CAMERA MOVEMENT' (with icons for WIDE SHOT, MEDIUM SHOT, CLOSE-UP, TOP-DOWN, PAN RIGHT, TILT DOWN, SLOW PUSH IN), 'VISUAL STYLE : Soft natural lighting, warm pastel tones, cozy aesthetic', 'NOTES FOR VIDEO GENERATION', and 'NEGATIVE PROMPT (HINDARI)'. Highly structured light layout, sharp typography, soft natural indoor lighting. ${faceClause}. --ar 3:4`;
  }

  if (style === 'unboxing_cinematic_grid') {
    return `A professional product unboxing commercial storyboard sheet, vertical layout, clean dark-charcoal background. At the top header, a prominent dark banner reading 'STORYBOARD - UNBOXING' and the bold product title in center with 'PART ${pageNum}', and metadata fields showing 'DURASI TOTAL : 10 DETIK', 'JUMLAH SCENE : ${endScene}', 'ASPECT RATIO : 16:9'. The main content is a structured 3x3 (9-shot) grid layout displaying scenes ${startScene} to ${endScene}: ${finalPromptText}. Each grid panel displays a widescreen 16:9 unboxing photo, a red/black square scene number badge (1, 2, 3...) at the top-left, a red timestamp badge (e.g. '0.0 - 1.0 DETIK') at the top-right, and a structured metadata block below with 3 fields: 'VISUAL : [Description]', 'AKSI : [Action Description]', and 'KAMERA : [Camera Movement]'. At the bottom, a prominent 5-box technical summary footer detailing 'INFORMASI TEKNIS', 'CAMERA MOVEMENT' (with icons for WIDE SHOT, MEDIUM SHOT, CLOSE-UP, TOP-DOWN, LOW ANGLE, TRACKING, PUSH-IN, HANDHELD), 'VISUAL STYLE', 'NOTES FOR VIDEO GENERATION', and 'NEGATIVE PROMPT (HINDARI)'. Highly structured dark layout, sharp typography, professional product studio lighting. ${faceClause}. --ar 3:4`;
  }

  if (style === 'ugc_overlay_card_grid') {
    return `A professional beauty/skincare UGC commercial script sheet, vertical layout. At the top header, a large bold product title banner with subtitle, and a top-right metadata card showing 'PLATFORM', 'DURASI', 'STYLE : BEAUTY INFLUENCER', 'HOOK STYLE : RELATABLE PROBLEM'. The body features stacked scene rows for scenes ${startScene} to ${endScene}: ${finalPromptText}. Each row displays on the left a 16:9 influencer visual panel with a dark 'SCENE [N]' badge at top-left, a timestamp badge at bottom-left ('00:00 - 00:02'), and a rounded white pill text overlay speech bubble badge at bottom-right of the image (e.g. 'Unboxing yuk! ✨'). On the right of each row is a clean metadata card box with 4 fields: 'SCENE TITLE', 'VISUAL', 'TEXT OVERLAY', and 'VOICE OVER'. At the bottom, 3 callout footer boxes: 'KENAPA HARUS [PRODUCT]?', 'INGAT!', and 'BELI SEKARANG!' with shopping cart icon. Clean aesthetic, soft studio lighting, sharp typography. ${faceClause}. --ar 3:4`;
  }

  if (style === 'ugc_overlay_dark_table') {
    return `A high-impact dark tech review UGC commercial script table sheet, vertical layout. At the top header, a heavy dark banner with bold product title, tagline, and top-right metadata box ('PLATFORM: YOUTUBE SHORTS', 'DURASI: 10 DETIK', 'GAYA: TECH REVIEW', 'HOOK STYLE: QUESTION'). Below is a gold-accented 4-column script table titled: 'SCENE', 'VISUAL & AKSI', 'TEXT OVERLAY', and 'VOICE OVER'. Rows display scenes ${startScene} to ${endScene}: ${finalPromptText}. Column 1 shows giant gold scene numbers (1, 2, 3...), timestamp, and scene stage badges ('HOOK', 'INTRO', 'KELEBIHAN', 'DEMO FITUR', 'VERDICT + CTA'). Column 2 shows high-res influencer demonstration photos. Column 3 shows bold sticker-style graphic overlay text ('WORTH IT ATAU ENGGAK?', 'BASS NENDANG!', 'CHECK OUT SEKARANG!'). Column 4 shows voiceover dialogue. At the bottom, a dark product showcase footer banner titled 'CATATAN PRODUK'. High-contrast dark theme, bold typography, professional lighting. ${faceClause}. --ar 3:4`;
  }

  if (style === 'ugc_overlay_minimal_clean') {
    return `A clean aesthetic talking head UGC commercial script sheet, vertical layout. At the top header, a hero product image frame on the left, bold title & italic tagline in center, and top-right metadata row with icons ('Platform', 'Duration', 'Style Talking Head', 'Hook Style Authority'). Below is a clean 6-column script table titled: 'SCENE', 'TIME', 'SCENE TITLE', 'VISUAL', 'TEXT OVERLAY', and 'VOICE OVER'. Rows display scenes ${startScene} to ${endScene}: ${finalPromptText}. Column 1 displays circular scene badges (1, 2, 3...) over presenter photos. Column 2 shows time interval ('00:00 - 00:02'). Column 3 shows scene title badges ('HOOK AWAL', 'ALASAN 1', 'RINGKASAN & CTA'). Column 4 shows visual description. Column 5 displays soft pastel rounded highlight overlay text badges ('3 Alasan Wajib Punya!', '1. Desain Elegant', 'Check Out Sekarang! 🛒'). Column 6 shows voiceover text. At the bottom, a clean lightbulb tip banner ('💡 TIPS: Gunakan tone ramah dan ekspresi natural'). Clean modern aesthetic, soft studio lighting. ${faceClause}. --ar 3:4`;
  }

  if (style === 'cinematic_matrix_grid') {
    return `A professional B-Roll Film & TVC Storyboard Matrix Grid presentation sheet, vertical layout sheet. At the top header, a clean black-and-white header banner reading 'STORYBOARD - B-ROLL COMMERCIAL' and 'PART ${pageNum}' with a metadata line: 'DURATION : ± 30-40 DETIK | STYLE : CINEMATIC B-ROLL | ASPECT RATIO : 16:9'. The main content is a structured 5-column matrix grid layout showing scenes ${startScene} to ${endScene}: ${finalPromptText}. Each grid panel contains a clean top header bar with numbered scene titles (e.g. '01. OPENING - WIDE', '02. PRODUCT HERO', '03. DETAIL TEXTURE', '04. OPEN PACK', '05. POWDER POUR'), a 16:9 cinematic image panel, and a clean text metadata block underneath with 4 structured rows: 'CAMERA : [Shot Type]', 'FOCUS : [Focus Target]', 'ACTION : [Motion & Action]', and 'NOTES : [Lighting & Audio Notes]'. At the very bottom, a clean 4-box technical summary footer detailing 'VISUAL STYLE', 'MOOD & TONE', 'MUSIC & SOUND', and 'TOTAL DURATION & PLATFORM'. High-contrast director storyboard matrix grid, clean line borders, sharp typography, cinematic sketch studio lighting. ${faceClause}. --ar 3:4`;
  }

  if (style === 'tiktok_script_table') {
    return `A professional TikTok/Reels commercial script table sheet presentation, vertical layout. At the top header, a prominent dark banner showing '10-SECOND TIKTOK AD STORYBOARD' and 'STORYBOARD PART ${pageNum}' with giant bold product title, tagline, 3 key benefit badges, and a prominent top-right hero product showcase box with a yellow starburst callout badge. Below the header is a high-contrast 5-column script table containing columns titled: 'SCENE', 'DURASI', 'VISUAL', 'VO / DIALOG', and 'TEXT ON SCREEN'. The table rows display scenes ${startScene} to ${endScene} showing: ${finalPromptText}. Column 1 shows bold scene index numbers (1, 2, 3...) and scene role labels ('PROBLEM / HOOK', 'PRODUCT INTRODUCTION', 'TRANSFORMATION', 'PRODUCT DETAILS', 'RECOMMENDATION / CTA'). Column 2 shows duration (e.g. '2 SEC'). Column 3 displays high-fidelity visual panels with zoom-in circle badges. Column 4 displays voiceover narration quotes. Column 5 displays bold text-on-screen graphic overlays and red call-to-action button badges ('BELI SEKARANG!'). At the very bottom, a clean footer bar with feature icon badges. Highly structured social ad table layout, sharp typography, professional studio lighting. ${faceClause}. --ar 3:4`;
  }

  if (style === 'infographic_step_guide') {
    return `A professional step-by-step commercial infographic poster, vertical layout sheet. At the top header, a prominent title banner showing 'STORYBOARD PART ${pageNum}' in bold vibrant colors. The layout is organized into ${gridLayout} with prominent green/gold circular numbered badges (1, 2, 3... up to ${endScene}) placed at the top-left of every panel indicating each step/scene in sequence. The panels show: ${finalPromptText}. Includes clear step-by-step tutorial text overlays, feature callouts, before-and-after curved transformation arrows, and product benefit checkmarks. At the very bottom, a clean footer bar with 3 circular feature icon badges ('Formula Extra Power', 'Natural Extraction', 'Safe & Tested'). Highly organized infographic poster layout, clean typography, vibrant studio lighting. ${faceClause}. --ar 3:4`;
  }
