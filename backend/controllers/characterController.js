...
async function generateCharacterSheetImage(req, res) {
  try {
    ...
    // Determine primary provider to try: default to Magica unless explicitly requested scenario
    const wantScenario = provider === 'scenario' && userRow && userRow.cus === 1;
    const wantMagica = !wantScenario;
